...
void* restoreAllDBs(void* arg)
{
    ...
    v_secure_system("rm -f /opt/secure/bootstrap.json");
...
#if defined(WAN_MANAGER_UNIFICATION_ENABLED)
void  Set_IPMode_AllInterfaces(ULONG value)
{
    ULONG                   ulIntTotal             = 0;
    ULONG                   ulVirIntTotal          = 0;
    ULONG                   ulIntIndex             = 0;
    ULONG                   ulVirIntIndex          = 0;
    errno_t                 rc                     = -1;
    char                    paramName[256]         = {0};
    CCSP_MESSAGE_BUS_INFO   *bus_info              = (CCSP_MESSAGE_BUS_INFO *)bus_handle;
    char*                   faultParam             = NULL;
    char*                   modes[3]               = {"IPv4 Only","IPv6 Only","Dual Stack"};
    int                     ret, nval;
    parameterValStruct_t    **valWanDML;
    char                    *parameterNames[1]     = {};

    rc = sprintf_s(paramName, sizeof(paramName), WAN_DML_IF_COUNT);
    if(rc < EOK)  
    {
        ERR_CHK(rc);
    }	
    parameterNames[0]  = paramName;
    ...
    if (CCSP_SUCCESS == ret) {
        ulIntTotal = atoi(valWanDML[0]->parameterValue);
    }

    if (valWanDML) {
        free_parameterValStruct_t (bus_handle, nval, valWanDML);
    }
	
    for ( ulIntIndex = 1; ulIntIndex <= ulIntTotal; ulIntIndex++ )
    {
        rc = sprintf_s(paramName, sizeof(paramName), WAN_DML_VIF_COUNT, ulIntIndex);
	...
	parameterNames[0]  = paramName;
        ...
        if (CCSP_SUCCESS == ret){
            ulVirIntTotal = atoi(valWanDML[0]->parameterValue);
        }
        ...
        for ( ulVirIntIndex = 1; ulVirIntIndex <= ulVirIntTotal; ulVirIntIndex++ )
        {
            memset(paramName,0,sizeof(paramName));

            rc = sprintf_s(paramName, sizeof(paramName), WAN_DML_VIF_IP_MODE, ulIntIndex,ulVirIntIndex);
            if(rc < EOK)
            {
                ERR_CHK(rc);
            }
            parameterValStruct_t  valueStruct = { paramName, modes[value-2], ccsp_string};
            ret = CcspBaseIf_setParameterValues(
                      bus_handle,
                      WAN_COMPONENT_NAME,
                      WAN_DBUS_PATH,
...
#if defined(WAN_MANAGER_UNIFICATION_ENABLED)
    if (value > COSA_DML_DEVICE_MODE_Bridge) //If set to bridge mode(value = 1), IP.Mode to be unchanged.
    {
        Set_IPMode_AllInterfaces(value);
    }
#endif
    value--;
