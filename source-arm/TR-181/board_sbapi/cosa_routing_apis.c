...
Route6_Add(const char *prefix, const char *gw, const char *dev, int metric)
{
    ...
    if (gw && *gw) {
        if (metric != 0)
#ifdef CORE_NET_LIB
            ret = route_add_va_arg("-6 %s dev %s via %s metric %d", prefix, dev, gw, metric);
#else
            ret = v_secure_system("ip -6 route add %s dev %s via %s metric %d", prefix, dev, gw, metric);
#endif /* CORE_NET_LIB */
        else
#ifdef CORE_NET_LIB
            ret = route_add_va_arg("-6 %s dev %s via %s", prefix, dev, gw);
#else
        ...
        if (metric != 0)
#ifdef CORE_NET_LIB
            ret = route_add_va_arg("-6 %s dev %s metric %d", prefix, dev, metric);
#else
            ret = v_secure_system("ip -6 route  add %s dev %s metric %d", prefix, dev, metric);
#endif /* CORE_NET_LIB */
        else
#ifdef CORE_NET_LIB
            ret = route_add_va_arg("-6 %s dev %s", prefix, dev);
#else
...
Route6_Del(const char *prefix, const char *gw, const char *dev)
{
    ...
    if (dev && *dev) {
        if (gw && *gw)
#ifdef CORE_NET_LIB
            ret = route_delete_va_arg("-6 %s dev %s via %s", prefix, dev, gw);
#else
            ret = v_secure_system("ip -6 route del %s dev %s via %s", prefix, dev, gw);
#endif /* CORE_NET_LIB */
        else
#ifdef CORE_NET_LIB
            ret = route_delete_va_arg("-6 %s dev %s", prefix, dev);
#else
        ...
        if (gw && *gw)
#ifdef CORE_NET_LIB
            ret = route_delete_va_arg("-6 %s via %s", prefix, gw);
#else
            ret = v_secure_system("ip -6 route del %s via %s", prefix, gw);
#endif /* CORE_NET_LIB */
        else
#ifdef CORE_NET_LIB
            ret = route_delete_va_arg("-6 %s ", prefix);
#else
    ...
    )	
{
    ...
    {
#ifdef CORE_NET_LIB
        if((err = route_add_va_arg("%d.%d.%d.%d/%d via %s %s %s ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)), "dev", pEntry->Interface)) != 0) {
#else
    ...
    {
#ifdef CORE_NET_LIB
        if((err = route_add_va_arg("%d.%d.%d.%d/%d via %s ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)))) != 0) {
#else
    ...
    {
#ifdef CORE_NET_LIB
        if((err = route_add_va_arg("%d.%d.%d.%d/%d via %s table erouter metric %d ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)), metric)) != 0) {	    
#else
    ...
    )
{
    ...
    for(index = 0; index < Config_Num; index++)
    {
        ...
        {
#ifdef CORE_NET_LIB
            err = route_add_va_arg("%d.%d.%d.%d/%d via %s %s %s ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)), "dev", pEntry->Interface);
#else
        ...
        {
#ifdef CORE_NET_LIB
            err = route_add_va_arg("%d.%d.%d.%d/%d via %s ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)));
#else
        ...
        {
#ifdef CORE_NET_LIB
            err = route_add_va_arg("%d.%d.%d.%d/%d via %s table erouter metric %d ", pEntry->DestIPAddress.Dot[0], pEntry->DestIPAddress.Dot[1], pEntry->DestIPAddress.Dot[2], pEntry->DestIPAddress.Dot[3], Count_NetmaskBitNum(pEntry->DestSubnetMask.Value), inet_ntoa(*(struct in_addr *)&(pEntry->GatewayIPAddress.Value)), metric);
#else
